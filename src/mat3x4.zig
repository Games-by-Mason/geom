...
pub const Mat3x4 = extern struct {
    ...
    /// space.
    pub fn ortho(frustum: Frustum3) @This() {
    ...
    test ortho {
        ...
        const m = ortho(f);
        try expectVec3ApproxEql(.{ .x = 1.0, .y = 2.0 / 6.0, .z = 1.0 / 3.0 }, m.timesPoint(.zero));
        try expectVec3ApproxEql(.{ .x = 1, .y = 1, .z = 1 }, m.timesPoint(.{ .x = 0, .y = -2, .z = 2 }));
        try expectVec3ApproxEql(.{ .x = -1, .y = -1, .z = 0 }, m.timesPoint(.{ .x = -2, .y = 4, .z = -1 }));
    }
    ...
    pub fn timesPoint(self: @This(), v: Vec3) Vec3 {
        return .{
            .x = self.r0.w + @mulAdd(f32, self.r0.z, v.z, @mulAdd(f32, self.r0.x, v.x, self.r0.y * v.y)),
            .y = self.r1.w + @mulAdd(f32, self.r1.z, v.z, @mulAdd(f32, self.r1.x, v.x, self.r1.y * v.y)),
            .z = self.r2.w + @mulAdd(f32, self.r2.z, v.z, @mulAdd(f32, self.r2.x, v.x, self.r2.y * v.y)),
        };
    }
    ...
    pub fn timesDir(self: @This(), v: Vec3) Vec3 {
        return .{
            .x = @mulAdd(f32, self.r0.z, v.z, @mulAdd(f32, self.r0.x, v.x, self.r0.y * v.y)),
            .y = @mulAdd(f32, self.r1.z, v.z, @mulAdd(f32, self.r1.x, v.x, self.r1.y * v.y)),
            .z = @mulAdd(f32, self.r2.z, v.z, @mulAdd(f32, self.r2.x, v.x, self.r2.y * v.y)),
        };
    }
